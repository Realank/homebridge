...
API.prototype.accessory = function(name) {
  ...
  if (name.indexOf('.') == -1) {
    ...
    if (found.length == 1) {
      ...
      var errStr = "The requested accessory '" + name + "' has been registered multiple times. Please be more specific by writing one of: " + found.join(", ");
      throw new Error(errStr);
    }
    else {
      var errStr = "The requested accessory '" + name + "' was not registered by any plugin.";
      throw new Error(errStr);
    }
  }
  else {
...
API.prototype.registerAccessory = function(pluginName, accessoryName, constructor, configurationRequestHandler) {
  ...
  if (this._accessories[fullName]){
    ...
    var errStr = "Attempting to register an accessory '" + fullName + "' which has already been registered!";
      throw new Error(errStr);
  }
...
API.prototype.publishCameraAccessories = function(pluginName, accessories) {
  for (var index in accessories) {
    ...
    if (!(accessory instanceof PlatformAccessory)) {
      var errStr = pluginName + " attempt to register an accessory that isn\'t PlatformAccessory!";
      throw new Error(errStr);
    }
...
API.prototype.platform = function(name) {
  ...
  if (name.indexOf('.') == -1) {
    ...
    if (found.length == 1) {
      ...
      var errStr = "The requested platform '" + name + "' has been registered multiple times. Please be more specific by writing one of: " + found.join(", ");
      throw new Error(errStr);
    }
    else {
      var errStr = "The requested platform '" + name + "' was not registered by any plugin.";
      throw new Error(errStr);
    }
  }
  else {
...
API.prototype.registerPlatform = function(pluginName, platformName, constructor, dynamic) {
  ...
  if (this._platforms[fullName]){
    var errStr = "Attempting to register a platform '" + fullName + "' which has already been registered!";
    throw new Error(errStr);
  }
...
API.prototype.registerPlatformAccessories = function(pluginName, platformName, accessories) {
  for (var index in accessories) {
    var accessory = accessories[index];
    if (!(accessory instanceof PlatformAccessory)) {
      var errStr = pluginName + " - " + platformName + " attempt to register an accessory that isn\'t PlatformAccessory!";
      throw new Error(errStr);
    }
    accessory._associatedPlugin = pluginName;
...
API.prototype.unregisterPlatformAccessories = function(pluginName, platformName, accessories) {
  for (var index in accessories) {
    var accessory = accessories[index];
    if (!(accessory instanceof PlatformAccessory)) {
      var errStr = pluginName + " - " + platformName + " attempt to unregister an accessory that isn\'t PlatformAccessory!";
      throw new Error(errStr);
    }
  }
